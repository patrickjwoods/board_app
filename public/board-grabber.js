...
/* via https://github.com/tomrandle/imagebookmarklet */
...
function runthis() {
	...
	jQuery(document).ready(function(){
		...
		if(jQuery('#image-grabber-container').length == 0) {
			...
			jQuery('<style type="text/css">@import url("http://boardmaker.herokuapp.com/grabber-style.css");</style>').appendTo("head");
	...
	function addImage( imageToAdd ) {
		...
		if(imageWidth > 150 && imageHeight > 200 && containData === -1) {
			...
			var finalLink =  beginLiTag + imageURL + endATag + beginImageTag + imageURL + middleImageTag + calculatedMargin + endImageTag + "<span>" + imageWidth + " x " + imageHeight + "</span>" + endLiTag + "<p class='src-url'>" + imageURL + "</p>";
